def create_lines(wire):
    ...
    for instr in wire:
        ...
        if direction == "L":
            new_cord = (prev_cord[0] - magnitude, prev_cord[1])
        elif direction == "R":
            new_cord = (prev_cord[0] + magnitude, prev_cord[1])
        elif direction == "U":
            new_cord = (prev_cord[0], prev_cord[1] + magnitude)
        elif direction == "D":
            new_cord = (prev_cord[0], prev_cord[1] - magnitude)

        lines.append([prev_cord, new_cord, steps])
        steps += magnitude
        prev_cord = new_cord

    return lines

print(5 in range(8, 6-1, -1))
...
#wire1 = ["R8","U5","L5","D3"]
#wire2 = ["U7","R6","D4","L4"]
lines1 = create_lines(wire1)
lines2 = create_lines(wire2)

intersections = []
for l1 in lines1:
    for l2 in lines2:
        hor1, hor2, ver1, ver2 = False, False, False, False
        if l1[0][1] == l1[1][1]:
            hor1 = True
        elif l1[0][0] == l1[1][0]:
            ver1 = True
        ...
        if (hor1 == True and hor2 == True) or (ver1 == True and ver2 == True):
            # parallel shit
            pass
        else:
            if hor1:
            ...
            if hor[0][0] < hor[1][0]:
                condition1 = ver[0][0] in range(hor[0][0], hor[1][0]+1)
            else:
                condition1 = ver[0][0] in range(hor[0][0], hor[1][0]-1, -1)

            if ver[0][1] < ver[1][1]:
                condition2 = hor[0][1] in range(ver[0][1], ver[1][1]+1)
            else:
                condition2 = hor[0][1] in range(ver[0][1], ver[1][1]-1, -1)

            if condition1 and condition2:
                print("INTERSECTS!")

                intersection = (ver[0][0], hor[0][1])
                d1 = abs(ver[0][1] - intersection[1]) + ver[2]
                d2 = abs(hor[0][0] - intersection[0]) + hor[2]

                intersections.append([intersection, d1+d2])

#intersections.remove((0, 0))
#for count, coord in enumerate(intersections):
#    dist = abs(coord[0]) + abs(coord[1])
#    intersections[count] = [dist, coord]
