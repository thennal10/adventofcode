...
rotate = {1: 3,
          ...
          2: 4,
          4: 1}
inv_rotate = {v: k for k, v in rotate.items()}
def d2c(direction, c):
    coord = c.copy()
    if direction == 1:
        coord[1] += 1
...
for i in range(10000):
    repairbot.input = inv_rotate[hug]
    out = next(program)
    if out == 0:
        grid.add(tuple(d2c(inv_rotate[hug], coord) + [1]))
        repairbot.input = hug
        out = next(program)
        if out == 0:
            grid.add(tuple(d2c(hug, coord) + [1]))
            hug = rotate[hug]
        else:
            coord = d2c(hug, coord)
            if out == 1:
                grid.add(tuple(coord + [0]))
            elif out == 2:
                grid.add(tuple(coord + [2]))
    else:
        hug = inv_rotate[hug]
        coord = d2c(hug, coord)
        if out == 1:
...
def create(node):
    ...
    if [x[2] for x in grid if x[:2] == (crd[0], crd[1] + 1)][0] != 1 and node.parent.name != [crd[0], crd[1] + 1]:
        create(Node([crd[0], crd[1] + 1], parent=node))
        flg = True
    if [x[2] for x in grid if x[:2] == (crd[0], crd[1] - 1)][0] != 1 and node.parent.name != [crd[0], crd[1] - 1]:
        create(Node([crd[0], crd[1] - 1], parent=node))
        flg = True
    if [x[2] for x in grid if x[:2] == (crd[0] - 1, crd[1])][0] != 1 and node.parent.name != [crd[0] - 1, crd[1]]:
        create(Node([crd[0] - 1, crd[1]], parent=node))
        flg = True
    if [x[2] for x in grid if x[:2] == (crd[0] + 1, crd[1])][0] != 1 and node.parent.name != [crd[0] + 1, crd[1]]:
        create(Node([crd[0] + 1, crd[1]], parent=node))
        flg = True
