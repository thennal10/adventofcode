...
comp = Intcode(inp)
result1 = comp.run([1144, 905])
comp = Intcode(inp)
result2 = comp.run([1144 - 100, 905])
comp = Intcode(inp)
result3 = comp.run([1144 - 100, 905 + 100])
print(result1)
print(result2)
print(result3)
...
for i in range(10, 100000):
    ...
    for a in range(100):
        ...
        for k in range(100):
            comp = Intcode(inp)
            result = comp.run([i - k, jstart])
            if result == 0:
                print("ding")
                break
        else:
            comp = Intcode(inp)
            result = comp.run([i - 99, jstart + 99])
            if result == 1:
                print('YAHOO MOTHERFUCKER')
                print(f"{i}, {jstart}")
                break
